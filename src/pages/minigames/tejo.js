...
function getInitialTejoState() {
    return {
        ...
        vz: 0,
        radius: 22,
        state: "INIT", // INIT, DIR, POWER, FLYING, LANDED
        dirOsc: 0,
        powOsc: 0,
        hit: false,
        rotation: 0,
...
const ads = [
    { el: document.getElementById("ad-tejo"), x: 100, y: 100, vx: 150, vy: 150, width: 150, height: 100 },
    { el: document.getElementById("ad-tejo-2"), x: 300, y: 200, vx: -200, vy: 120, width: 200, height: 100 },
    { el: document.getElementById("ad-tejo-3"), x: 500, y: 50, vx: 100, vy: -180, width: 100, height: 100 }
];
...
function drawTejo() {
    ...
    // Shadow
    ctx.beginPath();
    ctx.ellipse(tejo.x, tejo.y + 8, tejo.radius * 0.9, tejo.radius * 0.3, 0, 0, Math.PI * 2);
...
function drawMeters() {
    if (tejo.state === "INIT" || tejo.state === "FLYING" || tejo.state === "LANDED") return;
...
function land() {
    ...
    const hitTarget = targets.find(t => distance(tejo.x, tejo.y, t.x, t.y) < t.radius);
